...
var htmx2 = (function() {
  ...
  function codeMatches(responseHandlingConfig, status2) {
    var regExp = new RegExp(responseHandlingConfig.code);
    return regExp.test(status2.toString(10));
  }
...
// node_modules/@webdyne/webdyne-zeroperl-browser/browser/htmx.js
function isHtmxScript(source, base2) {
  if (!source) return false;
  const url = new URL(source, base2);
  return /\/htmx(?:\.min)?\.js$/.test(url.pathname) && (url.origin === new URL(base2).origin || url.hostname === "cdn.jsdelivr.net" && url.pathname.startsWith("/npm/htmx.org") || url.hostname === "unpkg.com" && url.pathname.startsWith("/htmx.org"));
}
function htmxRequestUrl(path, current2, shell, applicationUrl2) {
  const url = new URL(path || shell, current2);
  const shellUrl = new URL(shell);
  if (url.origin === shellUrl.origin && url.pathname === shellUrl.pathname) {
    return new URL(current2).href;
  }
  return applicationUrl2(url).href;
}
function installHtmx({ page: page2, currentUrl, applicationUrl: applicationUrl2 }) {
  window.htmx = htmx_esm_default;
  const requests = /* @__PURE__ */ new Set();
  document.addEventListener("htmx:configRequest", (event) => {
    if (!page2.contains(event.detail.elt)) return;
    event.detail.path = htmxRequestUrl(event.detail.path, currentUrl(), location.href, applicationUrl2);
    event.detail.headers["HX-Current-URL"] = currentUrl();
  });
  document.addEventListener("htmx:beforeRequest", (event) => {
    if (page2.contains(event.detail.elt)) requests.add(event.detail.xhr);
  });
  ...
  return {
    ...
    clearPage() {
      for (const child of [...page2.children]) htmx_esm_default.remove(child);
      page2.replaceChildren();
    },
    process() {
      htmx_esm_default.process(page2);
    }
  };
}

// node_modules/@webdyne/webdyne-zeroperl-browser/browser/shell.js
var base = new URL("./", import.meta.url);
var virtual = new URL("__webdyne_app/", base);
var worker = new Worker(new URL("perl-worker.js", base), { type: "module" });
var page = document.getElementById("webdyne-page");
var status = document.getElementById("webdyne-status");
var baseElement = document.createElement("base");
document.head.prepend(baseElement);
var current = new URL(define_LOCAL_CONFIG_default.entry, virtual);
var navigation;
var nativeFetch = window.fetch.bind(window);
var htmxHost = installHtmx({ page, currentUrl: () => current.href, applicationUrl });
...
async function serviceWorkerReady() {
  ...
  if (!navigator.serviceWorker.controller) await new Promise((resolve, reject) => {
    const timer = setTimeout(() => reject(Error("Service worker did not take control")), 3e4);
...
async function navigate(url, init = {}, updateHistory = true) {
  ...
  for (let redirects = 0; redirects <= 10; redirects++) {
    ...
    if (!location2 || redirects === 10) throw Error("Invalid or excessive redirect");
    const redirected = new URL(location2, target);
    if (redirected.origin !== base.origin) {
      window.location.href = redirected.href;
      return;
    }
...
window.webdyneLocal = Object.freeze({ navigate, get url() {
  return current.href;
} });
function route() {
  return applicationUrl(location.hash.startsWith("#/") ? location.hash.slice(1) : define_LOCAL_CONFIG_default.entry);
}
